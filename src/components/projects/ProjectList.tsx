  ...
  return (
    <div className="min-h-screen bg-[#121212] text-white">
      <div>
        ...
        <StatsBar 
          stats={[
            { label: 'Total', value: projects.length, isMonospace: true },
            { label: '', value: formatCurrency(projects.reduce((sum, p) => sum + p.budget, 0)), isMonospace: true },
            ...(isConstrained ? [] : [
              { label: 'Active', value: projects.filter(p => p.status === 'active').length, subValue: `(${Math.round((projects.filter(p => p.status === 'active').length / projects.length) * 100)}%)`, color: 'green' as const, isMonospace: true },
              { label: 'Avg Budget', value: formatCurrency(projects.reduce((sum, p) => sum + p.budget, 0) / projects.length), color: 'default' as const, isMonospace: true }
            ])
          ]}
        />
